package com.brunnodanyel.workshopmongo.config;

import java.text.SimpleDateFormat;
import java.util.Arrays;
import java.util.TimeZone;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.CommandLineRunner;
import org.springframework.context.annotation.Configuration;

import com.brunnodanyel.workshopmongo.domain.Post;
import com.brunnodanyel.workshopmongo.domain.User;
import com.brunnodanyel.workshopmongo.repository.PostRepository;
import com.brunnodanyel.workshopmongo.repository.UserRepository;

@Configuration
public class Instantiation implements CommandLineRunner {

	@Autowired
	private UserRepository userRepository;

	@Autowired
	private PostRepository postRepostory;

	@Override
	public void run(String... args) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		sdf.setTimeZone(TimeZone.getTimeZone("GMT"));

		userRepository.deleteAll();
		postRepostory.deleteAll();

		User maria = new User(null, "Maria Brown", "maria@gmail.com");
		User alex = new User(null, "Alex Green", "alex@gmail.com");
		User bob = new User(null, "Bob Grey", "bob@gmail.com");

		Post post1 = new Post(null, sdf.parse("21/03/2018"), "Vou viajar para São Paulo. Abraços!", "Partiu viagem",maria);
		Post post2 = new Post(null, sdf.parse("23/03/2018"), "Acordei feliz hoje!", "Bom dia", maria);

		userRepository.saveAll(Arrays.asList(maria, alex, bob));
		postRepostory.saveAll(Arrays.asList(post1, post2));

	}

}
